...
// This camera moves along a Bezier but always looking tangentially.
...
cam.object.initialize = function () {
    ...
    this.up.set(0,1,0);
    var tan = cam.curve.getTangent(0);
    var lookPt = tan.add(cam.start);
    this.lookAt(lookPt);
}

cam.object.update = function (dt) {
    if( globalTime < 1 ) {
        var here = cam.curve.getPoint(globalTime);
        this.position.copy( here );
        var tan = cam.curve.getTangent(globalTime);
        var lookPt = tan.add(here);
        this.lookAt(lookPt);
    }
}
